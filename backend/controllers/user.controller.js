...
export const followUnFollowUser = async(req,res) => {
    try {
        ...
        if(id === req.user._id){
            return res.status(400).json({error:"you can't unfollow/follow you"})
        }
...
export const updateUser = async(req,res) => {
    try {
        ...
        if ((!newPassword && currentPassword)|| (!currentPassword && newPassword)){
            return res.status(400).json({error : "please provide both the new password and cureent password "})
        }
